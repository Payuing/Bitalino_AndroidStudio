...
public class BiopluxService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		String recordingName = intent.getStringExtra(HomeActivity.KEY_RECORDING_NAME).toString();
		configuration = (DeviceConfiguration) intent.getSerializableExtra(HomeActivity.KEY_CONFIGURATION);
		...
		if (connectToBiopluxDevice()) {
			startChronometer();
			dataManager = new DataManager(this, recordingName + currentDateandTime, configuration, "Patient", "Name");
			createNotification();
		}
	...
	private void createNotification() {
		...
		Intent newRecordingIntent = new Intent(this, HomeActivity.class);

		// PENDING INTENT
		PendingIntent pendingIntent = PendingIntent.getActivity(this, 0, newRecordingIntent, PendingIntent.FLAG_CANCEL_CURRENT);
	...
	private void sendErrorToActivity(int errorCode) {
		try {
			...
			Log.e(TAG,
					"Exception sending error message to activity. Service is stopping",e);
		}
	}
